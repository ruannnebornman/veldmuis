...
manifest_url="${PUBLISHED_AUR_MANIFEST_URL:-}"
work_root="${RUNNER_TEMP:-/tmp}/veldmuis-aur-refresh"
resolved_refs_file="${VELDMUIS_AUR_RESOLVED_REFS_FILE:-${work_root}/resolved-aur-refs.txt}"
published_manifest="${work_root}/published-aur-manifest.txt"
published_refs_file="${work_root}/published-aur-refs.txt"

log() {
  printf '[check-aur-package-refresh] %s\n' "$*"
}

die() {
  printf '[check-aur-package-refresh] ERROR: %s\n' "$*" >&2
  exit 1
}
...
write_summary() {
...

  {
    ...
    echo "- Published manifest: ${manifest_url}"
    echo
...
finish() {
  ...
  write_output "manifest_url" "${manifest_url}"
...
main() {
  ...
  if [[ -z "${manifest_url}" ]]; then
    ...
    manifest_url="${package_base}/veldmuis-aur-packages.manifest.txt"
  fi
  ...
  if ! curl --fail --silent --show-error --location "${manifest_url}" \
  ...
  parse_manifest_refs "${published_manifest}" > "${published_refs_file}"
  ...
  if cmp -s "${resolved_refs_file}" "${published_refs_file}"; then
    finish "false" "Published AUR refs already match resolved refs."
  else
